...
- **Never commit to `master`.** Never push to the session's generated branch either (such as
  `claude/<words>`) — it is not a category and it names nothing. Create the real branch before
  the first commit:
